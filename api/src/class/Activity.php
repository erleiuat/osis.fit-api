...
class Activity extends ApiObject {

    /* -------- TABLES (T) AND VIEWS (V) -------- */
    private $t_main = "user_activity";

    /* ----------- BASIC PARAMS ---------- */
    protected $keys = ['id', 'title', 'duration', 'calories', 'stamp', 'date', 'time'];
    ...
    public function create() {
        ...
        $stmt = $this->db->conn->prepare("
            ...
            (`user_id`, `title`, `duration`, `calories`, `stamp`) VALUES 
            (:user_id, :title, :duration, :calories, :stamp);
        ");

        $this->db->bind($stmt, 
            ['user_id', 'title', 'duration', 'calories', 'stamp'],
            [$this->user->id, $this->title, $this->duration, $this->calories, $this->stamp]
        )->execute($stmt);
